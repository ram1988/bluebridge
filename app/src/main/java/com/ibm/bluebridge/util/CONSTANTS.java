package com.ibm.bluebridge.util;

/**
 * Created by huangq on 6/11/2015.
 */
public interface CONSTANTS {
    String PREFERENCE_NAME = "com.ibm.bluebridge";

    //String BM_ROUTE = "http://school-volunteer-bluebridge.mybluemix.net/";
    String BM_ROUTE = "http://bluebridge.mybluemix.net/";
    String BM_UUID = "4f2f35d9-105e-47af-9394-4036c8af47f5";
    String[] CATEGORIES = {"Education", "Sports", "Music", "Charity" , "Others"};
    String LOADING_DIALOG_TITLE = "Please Wait...";
    String LOADING_EVENTS_MSG = "Loading Events...";
    String LOADING_PARENTS_MSG = "Loading Parents list...";
    String ADDING_EVENT_MSG = "Adding Event...";
    String UPDATING_EVENT_MSG = "Updating Event...";
    String DELETING_EVENT_MSG = "Deleting Event...";

    String GCM_SENDER_ID = "917065075762";
}
